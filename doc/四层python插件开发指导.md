...
​       在插件基本信息中有详细说明，上传插件包含三个文件分别是 zip/tar 包（即插件源码目录或文件的打包），工具元数据信息 tools.json 以及插件的配置信息 plugin.json 数据，在上述的插件示例中，可以直接将源码打包成 zip/tar 文件，打包方式可以直接使用本机 PC 的压缩工具即可。

对于**tools.json文件**，下边根据上述的 python 源码给定以下的元数据信息：
...
> **注意**：由于 python 不支持编译，当前未开发生成 python 四层规范的 tools.json 的工具，因此可以参考四层 java 插件开发指导，配置 pom 规范
> 
> ```xml
...
> 
> runnables：必须包含 FIT 字段，该字段下必须包含 fitableId 与 genericableId，这里两个值支持 FIT 调用，可以实现在前端界面准确的调用对应的实现。【fitableId 与 genericableId 仅支持数字、大小写字母以及 '-'、'_'、'*'、'.' 字符且长度在128以内】
> 
...
| 参数名      | 说明                                                                                                         |
| ----------- |------------------------------------------------------------------------------------------------------------|
| checksum    | 表示用户打包的源码插件文件的SHA-256的值，这里作为完整性校验的标识。**这里建议使用 git bash 执行 sha256sum.exe xxx.jar 生成该值**                     |
| description | 插件的描述信息。                                                                                                   |
| name        | 插件名称。                                                                                                      |
| type        | 插件的类型。                                                                                                     |
